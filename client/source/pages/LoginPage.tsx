...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: React.MouseEvent) => {
    ...
    const secretDigest = arrayBufferToHexString(await calcSecretKey(username, password));
    ...
    try {
      const { data } = await axios.post<InterfaceAuthLoginResponse>(
        `${CONFIG_BASE_URL}/api/auth/login`,
        {
          username: username,
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -translate-1/2 w-100 p-4">
      ...
      <form onSubmit={event => event.preventDefault} className="flex flex-col gap-y-6">
        <FormInputWithIcon
          type="text"
          name="username"
          value={username}
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            <LazyUnderlineLink
              ...
              link={`${CONFIG_ROOT_PATH}/auth/forgot`}
            />
          </div>
        </div>
        <button
        ...
        <div className="flex items-center justify-center gap-x-2">
          ...
          <LazyUnderlineLink text="Register" link={`${CONFIG_ROOT_PATH}/auth/register`} />
        </div>
      </form>
    </div>
  );
